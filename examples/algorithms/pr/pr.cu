...
void test_pr(int num_arguments, char** argument_array) {
  if (num_arguments != 3) {
    std::cerr << "usage: ./bin/<program-name> filename.mtx outputfile.txt" << std::endl;
    exit(1);
  }
  ...
  if (util::is_market(filename)) {
    ...
    std::cout << "num_v: " << csr.number_of_rows << " ,num_e: " << csr.number_of_nonzeros << std::endl;
  ...
  print::all_to_file(p, argument_array[2]);
